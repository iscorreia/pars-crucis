# HIGH PRIORITY

- Create Weapon **_logic_**
- Basic Weapon/Gear **_hbs style/css_**

## IMPORTANT

- Persona Ability calculations: how to interact with weapons? Weapon abilities? **_logic_**
- Basic Ability **_hbs style/css_**
- Persona Inventory calculations **_logic_**
- Improved dice roll **_logic_**
- Currency block on Persona Inventory Tab **_feature_**

## NECESSARY

- Improve PASSIVE SHEET **_logic hbs style/css_**
- Improve PERSONA SHEET: PASSIVES TAB **_hbs style/css_**
- Implement usable Ability/Item actions **_logic_**
- Automatically set token actor-link/vision for personas **_config_**
- Create Non Playable Character - PDM sheet from Persona Sheet **_config_**
- Persona sheet Inventory Tab **_hbs style/css_**
- Improve Persona sheet Ability Tab: specially actions **_hbs style/css_**

## LOW PRIORITY

- Improved favorable skills on Persona Config **_logic style/css_**
- Improved Ability **_hbs style/css_**
- Improved Weapon/Gear **_hbs style/css_**
- Show Abilities/Items on chat **_feature_**
- Calculate AR / Robust / Insulant / AB based on equipped Gear/Passives/other **_logic_**
- Notes on the Persona Ability Tab **_feature update_**
- Weapons Compendium **_pack_**
- Items Compendium **_pack_**
- Abilities Compendium **_pack_**
- Passives Compendium **_pack_**

### INTERESTING BUT NOT NECESSARY

- Implement `enrichHTML` in the item descriptions/effect **_feature_**
- Skills/Abilities/Items Macros **_feature_**
- Persona Icon on chat **_feature update_**
- Tooltips / Information on hover **_feature_** or;
- Information icon: on click: pops a window with origin/skills/etc description **_feature_**
- Implement currency working on items

### MIGHT BE INTERESTING

- Custom effects **_feature_**
